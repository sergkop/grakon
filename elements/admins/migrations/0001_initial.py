...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.rename_table('elements_entityadmin', 'admins_entityadmin')

        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='elements', model='EntityAdmin').update(app_label='elements.admins')

    def backwards(self, orm):
        db.rename_table('admins_entityadmin', 'elements_entityadmin')
        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='elements.admins', model='EntityAdmin').update(app_label='elements')
